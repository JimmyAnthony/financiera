...
function WriteHTML2($html){
	...
	foreach($a as $i=>$e)
	{
		...
		else
		{
			//Tag
			if($e{0}=='/')
				$this->CloseTag2(strtoupper(substr($e,1)));
			else
			{
				//Extract attributes
				$a2=explode(' ',$e);
				...
				$this->OpenTag2($tag,$attr);
			}
		}
	}
}

function OpenTag2($tag,$attr)
{
	//Opening tag
	if($tag=='Btab' or $tag=='Itab' or $tag=='Utab')
		$this->SetStyle2($tag,true);
	if($tag=='A')
		$this->HREFtab=$attr['HREFtab'];
	if($tag=='BR')
		$this->Ln(5);
	if($tag=='P')
		$this->Ln(10);
...
function SetStyle2($tag,$enable)
{
	...
	$this->$tag+=($enable ? 1 : -1);
	$style='';
	foreach(array('Btab','Itab','Utab') as $s)
		if($this->$s>0)
			$style.=$s;
	$this->SetFont('',$style);
}

function PutLink2($URL,$txt)
{
	//Put a hyperlink
	$this->SetTextColor(0,0,255);
	$this->SetStyle2('Utab',true);
	$this->Write(5,$txt,$URL);
	$this->SetStyle2('Utab',false);
	$this->SetTextColor(0);
}
